...
void Eventer::_clearAll()	// 将数据成员重置，用于构造函数
{
  ch = -1;
  sid = -1;
  cid = -1;
...
bool Eventer::_openFile(const char * fileN)
{
...
bool Eventer::_readWord()
{
  if(fd < 0){
    printf("error, no opened file found!\n");
    return false;
  }
  size_t n = read(fd, &buff, 4); // 从fd中读取4个字节，写入buff
  if(n <= 0){
    return false;
  }
...
void Eventer::_printWord()
{
  printf("0x%08x\n", buff);
}

bool Eventer::_getNextWord()
{
  return _readWord();
}

bool Eventer::_getNextEvent()
{
  _clearAll();
  if(!_getNextWord()){		// 判断是否达到文件末尾
    return false;	       
  }
